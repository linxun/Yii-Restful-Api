	...
	public function insert(array $attributes=null)
	{
		...
		if($this->beforeSave())
		{
			...
			if(version_compare(Mongo::VERSION, '1.0.5','>=') === true)
				...
				$result = $this->getCollection()->insert($rawData, CPropertyValue::ensureBoolean('w'));//$this->getSafeFlag()));
		...
		foreach($data as $attributes)
		{
			if(($record=$this->populateRecord($attributes,$callAfterFind))!==null)
			{
				if($index===null)
					...
					$records[(string)$record->$index]=$record;    //modify this line to allow '_id' as $index
			}
		}
		return $records;
	}
