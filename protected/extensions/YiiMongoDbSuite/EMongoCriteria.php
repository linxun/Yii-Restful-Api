...
class EMongoCriteria extends CComponent
{
			...
			if(isset($criteria['conditions']))
				foreach($criteria['conditions'] as $fieldName=>$conditions)
				{
					...
					foreach($conditions as $operator => $value)
					{
						...
						//$operator = strtolower($operator);       //commented by kexianbing, because Mongodb require uppercase operator, for example maxDistance
	...
	public function __call($fieldName, $parameters)
	{
		if(isset($parameters[0]))
			...
			$operatorName = $parameters[0];    //modified by kexianbing, because Mongodb require uppercase operator, for example maxDistance
